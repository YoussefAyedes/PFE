...
export const applyForJob = async (req, res) => {
  ...
  const userId = req.user?._id; // Get user ID from the authenticated user (via middleware)

  if (!userId) {
    return res.status(400).json({ success: false, message: "Invalid user auth" });
  }
  ...
  try {
    ...
    // Add user ID to the application's array
