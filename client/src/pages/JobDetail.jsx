  ...
  const handleDeletePost = async () => {
    ...
    try {
        if (window.confirm("Delete Job Post?")) {
            ...
            console.log(res); // Debugging: Check the response here

            if (res?.success) {
                alert(res?.message);
                window.location.replace("/");
            } else {
                alert("Failed to delete the job post: " + res?.message);
            }
        }
    } catch (error) {
...
const handleApplyNow = async () => {
  ...
  try {
      const response = await apiRequest({
          url: "/jobs/apply-job",
          method: "POST",
          data: { jobId: job._id }, // Send job ID
          token: user?.token, // Include user token for authentication
      });

      if (response.success) {
          alert(response.message); // Show success message
          // Optional: Update UI or job application count
      } else {
          alert("Failed to apply: " + response.message); // Show error message
      }
  } catch (error) {
      console.error("Error applying for job:", error);
      alert("An error occurred while applying: " + error.message || "Unknown error");
  } finally {
      setIsFetching(false);
  }
};
      ...
      <div className='w-full flex flex-col md:flex-row gap-10'>
        ...
        <div className='w-full h-fit md:w-2/3 2xl:2/4 bg-white px-5 py-10 md:px-10 shadow-md'>
          ...
          <div className='w-full'>
            ...
            <CustomButton
            title='Apply Now'
            onClick={handleApplyNow}  // Call the apply function here
